...
/*   Updated: 2025/09/15 13:48:07 by oalananz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_pixels(t_draw *draw, mlx_texture_t *tex, t_raycast *raycast, int x)
{
	...
	while (y < draw->draw_end)
	{
		...
		draw->index = (draw->tex_y * tex->width + draw->tex_x)
			* tex->bytes_per_pixel;
		mlx_put_pixel(raycast->img, x, y, color_validation(tex, draw));
		y++;
	}
}
...
mlx_texture_t	*setup_wall_texture(t_draw *draw, t_raycast *raycast)
{
	mlx_texture_t	*tex;
...
void	draw_walls(t_raycast *raycast)
{
	...
	while (x < WINDOW_WIDTH)
	{
		...
		raycast_step_setup(draw, raycast);
		perform_dda(draw, raycast);
		draw_calculations(draw, raycast);
		tex = setup_wall_texture(draw, raycast);
